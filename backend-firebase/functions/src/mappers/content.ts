...
const normalizeHome = (data: any): any => {
  ...
  const heroSlides = Array.isArray(data.hero?.slides)
    ? data.hero.slides.map((slide: any) => ({
        ...slide,
        image: toPublicMediaUrl(slide?.image) ?? slide?.image,
      }))
    : data.hero?.slides;
    ...
    ? data.sponsors.tiers.map((tier: any) => ({
        ...
        sponsors: Array.isArray(tier?.sponsors)
          ? tier.sponsors.map((sponsor: any) => ({
              ...sponsor,
              logo: toPublicMediaUrl(sponsor?.logo) ?? sponsor?.logo,
            }))
          : tier?.sponsors,
      }))
    : data.sponsors?.tiers;
...
const normalizeAbout = (data: any): any => {
  ...
  const valueItems = Array.isArray(data.values?.items)
    ? data.values.items.map((item: any) => ({
        ...item,
        image: toPublicMediaUrl(item?.image) ?? item?.image,
      }))
    : data.values?.items;
  ...
  return {
    ...
    header: data.header
      ? {
          ...data.header,
          image: toPublicMediaUrl(data.header.image) ?? data.header.image,
        }
      : data.header,
    values: data.values
      ? {
...
const normalizeApplyStudent = (data: any): any => {
  ...
  return {
    ...
    hero: data.hero
      ? {
          ...data.hero,
          image: toPublicMediaUrl(data.hero.image) ?? data.hero.image,
        }
      : data.hero,
    intro: data.intro
      ? {
          ...data.intro,
          image: toPublicMediaUrl(data.intro.image) ?? data.intro.image,
        }
      : data.intro,
  };
};
...
const normalizeSiteSettings = (data: any): any => {
  ...
  return {
    ...
    branding: data.branding
      ? {
          ...data.branding,
          logo: toPublicMediaUrl(data.branding.logo) ?? data.branding.logo,
        }
      : data.branding,
  };
};
